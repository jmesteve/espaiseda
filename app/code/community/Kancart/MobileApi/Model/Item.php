...
class Kancart_MobileApi_Model_Item extends Kancart_MobileApi_Model_Abstract {
     ...
     * @return type
     */
    public function kancart_item_get() {
...
//add
    ...
    private function getProductBundleOptions(Mage_Catalog_Model_Product $product) {
        ...
        if ($product->getTypeInstance(true)->hasOptions($product)) {
            ...
            foreach ($bundleOptions as $_option) {
                ...
                $attribute['options'] = array();
                if (!$attribute['required'] && $attribute['input'] != self::OPTION_TYPE_MULTIPLE_SELECT) {
                    $none = array(
                        'attribute_id' => 'none',
                        'option_id' => '',
                        'title' => 'None',
    ...
    private function getProductCustomOptionsOption(Mage_Catalog_Model_Product $product) {
        ...
        foreach ($product->getOptions() as $option) {
            ...
            if (!$optionObj['required'] && $optionObj['input'] != self::OPTION_TYPE_MULTIPLE_SELECT) {
                $none = array(
                    'attribute_id' => 'none',
                    'option_id' => '',
                    'title' => 'None',
        ...
        if (!$product->isGrouped()) {
            ...
            if ($DisplayMinimalPrice && $_minimalPriceValue && $_minimalPriceValue < $_finalPrice) {
                ...
                if ($_finalPrice == $_price) {
                    $this->addtoDisplayPrices($display_prices, 'Price: ', $this->getCurrencyPrice($_price));
                } else {
